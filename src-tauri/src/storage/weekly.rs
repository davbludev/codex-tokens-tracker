...
impl Store {
    ...
    pub(crate) fn weekly_at(
        ...
        let mut excluded_samples = 0;
        {
            // SQLite sorts the canonical source chronology; Rust retains only a
            // single tie group and a bounded page. Raw metadata remains unchanged.
            let mut statement = tx.prepare("SELECT weekly_seconds(timestamp) AS seconds,weekly_nanos(timestamp) AS nanos,used_percent,resets_at FROM limit_samples WHERE bucket='codex' AND window_minutes=10080 ORDER BY seconds,nanos")
                .map_err(|_| ReadError::Storage)?;
            ...
            while let Some(row) = rows.next().map_err(|_| ReadError::Storage)? {
                ...
                let used = encoded.as_deref().and_then(dto::percentage);
                if let (Some(seconds), Some(nanos), Some(used)) = (seconds, nanos, used) {
