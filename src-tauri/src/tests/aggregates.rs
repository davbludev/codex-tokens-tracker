...
fn aggregates_merge_only_confirmed_worktrees_and_migrate_read_indexes() {
    ...
    store.connection().execute_batch("DROP TABLE model_price_backfills; DROP TABLE pricing_work; DROP TABLE observation_valuations; DROP TABLE model_price_versions; DROP TABLE detected_models; DROP INDEX observation_pricing_model; DROP INDEX session_effective_children; DROP INDEX observation_model_bucket; ALTER TABLE turn_contexts DROP COLUMN effort; ALTER TABLE observations DROP COLUMN effort; PRAGMA user_version=5;").unwrap();
    drop(store);
    let mut store = Store::open(&db).unwrap();
    assert_eq!(total(&global(&mut store)), Some("135"));
