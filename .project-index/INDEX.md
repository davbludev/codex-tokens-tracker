# Project navigation

- [Codex source research](codex-sources.md) — Source evidence and metadata-only fixtures for ingestion.
- [Live ingestion](live-ingestion.md) — Incremental source reading, accounting, storage, and desktop snapshot.
- [Usage aggregates](aggregates.md) — Prepared direct/inclusive summaries and bounded query delivery.
- [Project and model analytics](analytics.md) — Project comparisons, model catalog metrics and bounded history.
- [Session details interface](session-details-ui.md) — Integration route reserved for the session details interface area.
- [Model pricing](pricing.md) — Exact rates, immutable valuations, and bounded pricing persistence.
- [Weekly observations](weekly.md) — Cycles, comparable cost intervals, and exact percentage estimates.
- [Frontend dependencies](frontend-dependencies.md) — npm packages and frontend build entry point.
- [Settings and export](settings-export.md) — Source preferences, diagnostics, and streaming CSV delivery.
