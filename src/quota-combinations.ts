...
// The permanent components exclude what the optional badges add back, so a
// hypothesis never counts the same token twice.
export const combinations = [0, 1, 2, 4, 3, 5, 6, 7].map(mask => ({ mask, name: names[mask], color: colors[mask], components: ["Uncached input", "Visible output", ...optional.filter((_, bit) => mask & (1 << bit))] }));
...
export function formatted(value: string | null, money = false) {
  ...
  return (money ? "$" : "") + BigInt(whole).toLocaleString() + (fraction ? separator + fraction : "");
}
...
export function combinationStats(intervals: QuotaInterval[], mask: number, writes: WriteInterpretation) {
  ...
  return {
    ...
    // Token counts are whole tokens; only monetary ratios carry decimals.
    tokens: usable.length ? perPercent(tokens, percent, 0, 0) : null,
