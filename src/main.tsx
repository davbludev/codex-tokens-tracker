...
function App() {
  ...
  return <main>
    ...
    <nav className="app-navigation" aria-label="Usage views">{(["dashboard", "sessions", "projects", "models", "weekly", "settings"] as const).map(item => <button key={item} type="button" aria-pressed={view === item} onClick={() => setView(item)}>{item === "weekly" ? "Weekly History" : item[0].toUpperCase() + item.slice(1)}</button>)}</nav>
    {view === "dashboard" ? <Dashboard /> : view === "sessions" ? <Sessions /> : view === "projects" ? <Projects /> : view === "models" ? <Models /> : view === "weekly" ? <WeeklyHistory /> : <Settings onOpenPricing={() => setPricingOpen(true)} />}
